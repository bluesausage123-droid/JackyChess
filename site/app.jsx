        ...
        const CHESS_DB = {
            'r_king': {
                name: '帥', color: 'red', type: 'king', score: 80,
                element: '金', virtue: '義', emotion: '憂慮悲觀',
            ...
            'b_king': {
                name: '將', color: 'black', type: 'king', score: 80,
                element: '金', virtue: '義', emotion: '憂慮悲觀',
                organ: '肺、支氣管、大腸、大腦中樞、神經傳導、氣血運行',
            ...
            'r_guard': {
                name: '仕', color: 'red', type: 'guard', score: 60,
                element: '金', virtue: '義', emotion: '憂慮悲觀',
            ...
            'b_guard': {
                name: '士', color: 'black', type: 'guard', score: 60,
                element: '金', virtue: '義', emotion: '憂慮悲觀',
                organ: '肺、支氣管、呼吸系統、鼻子過敏、大腸、皮膚',
                rep: '輔佐者、規劃者,講究規矩與邏輯',
            ...
            'r_minister': {
                name: '相', color: 'red', type: 'minister', score: 40,
                element: '火', virtue: '禮', emotion: '情緒起伏',
            ...
            'b_minister': {
                name: '象', color: 'black', type: 'minister', score: 40,
                element: '火', virtue: '禮', emotion: '情緒起伏',
            ...
            'r_rook': {
                name: '俥', color: 'red', type: 'rook', score: 30,
                element: '木', virtue: '仁', emotion: '壓抑、浮躁、憤怒',
            ...
            'b_rook': {
                name: '車', color: 'black', type: 'rook', score: 30,
                element: '木', virtue: '仁', emotion: '壓抑、浮躁、憤怒',
                organ: '肝、膽、筋、目',
                rep: '效率派、講求結果、人裡聲音大',
            ...
            'r_knight': {
                name: '傌', color: 'red', type: 'knight', score: 20,
                element: '木', virtue: '仁', emotion: '壓抑、浮躁、憤怒',
            ...
            'b_knight': {
                name: '馬', color: 'black', type: 'knight', score: 20,
                element: '木', virtue: '仁', emotion: '壓抑、浮躁、憤怒',
                organ: '關節、骨折、肝',
                rep: '為了目標奔波、業務、需四處走動',
                pos: '靈活、適應力強、人脈廣',
            ...
            'r_cannon': {
                name: '炮', color: 'red', type: 'cannon', score: 15,
                element: '水', virtue: '智', emotion: '恐懼、疑慮',
            ...
            'b_cannon': {
                name: '包', color: 'black', type: 'cannon', score: 15,
                element: '水', virtue: '智', emotion: '恐懼、疑慮',
            ...
            'r_pawn': {
                name: '兵', color: 'red', type: 'pawn', score: 10,
                element: '土', virtue: '信', emotion: '思慮、在意、緊張',
            ...
            'b_pawn': {
                name: '卒', color: 'black', type: 'pawn', score: 10,
                element: '土', virtue: '信', emotion: '思慮、在意、緊張',
                organ: '脾、胃、腸胃道、口腔、肌肉',
                rep: '保守踏實的小兵',
                pos: '穩健、不冒險、安全感',
        ...
        const canAttack = (fromPos, toPos, attacker, all) => {
            ...
            switch (attacker.type) {
                case 'knight': // 馬傌走斜
                    ...
                    return ADJACENT_QUADRANTS.some(([a,b]) =>
                        (a === fromPos && b === toPos) || (b === fromPos && a === toPos)
                    );
                case 'cannon': // 炮包隔山
                    if (fromPos === 'center') return false; // 炮在中間沒有山可打
                    return toPos === OPPOSITE[fromPos];     // 隔中央打到對面
                case 'pawn':   // 兵卒只進不退:可往「上 / 左 / 右」,不可往下
                    if (fromPos === 'center') {
                        // 中央可吃上/左/右,不可吃下(往下=退)
        ...
        const computeScores = (pieces) => {
            ...
            let harvest = 0, sacrifice = 0;
            const harvestDetail = [], sacrificeDetail = [];
            ...
            for (const [fromPos, attacker] of Object.entries(pieces)) {
                ...
                for (const [toPos, target] of Object.entries(pieces)) {
                    ...
                    if (attacker.color === myColor) {
                        harvest += score; harvestDetail.push(detail);
                    } else {
                        sacrifice += score; sacrificeDetail.push(detail);
                    }
                }
            }

            return { harvest, sacrifice, harvestDetail, sacrificeDetail };
        };
        ...
        const PATTERNS = [
            {
                ...
                detect(p) {
                    ...
                    if (c.type === 'knight') {
                        ...
                        for (const [a,b] of ADJACENT_QUADRANTS) {
                            const A = p[a], B = p[b];
                            if (A.name === '馬' && B.name === '傌') count++;
                            else if (A.name === '傌' && B.name === '馬') count++;
                        }
                    } else if (c.type === 'cannon') {
                        // 包炮需相隔(對面)
                        if (p.up.name === FRIEND_MAP[p.down.name] && p.up.color !== p.down.color &&
                            (p.up.type === 'cannon')) count++;
                        if (p.left.name === FRIEND_MAP[p.right.name] && p.left.color !== p.right.color &&
                            (p.left.type === 'cannon')) count++;
                    } else {
                ...
                narrate(m, p) {
                    const c = p.center;
                    const adj =
                        c.type === 'guard'    ? '士仕為最佳的好朋友(靈魂伴侶)' :
                        c.type === 'minister' ? '象相為次佳的好朋友(修行夥伴)' :
                        c.type === 'rook'     ? '車俥相見容易各持己見,但情誼深厚' :
                        c.type === 'pawn'     ? '兵卒踏實的好朋友,如老夫老妻' :
                        c.type === 'knight'   ? '馬傌斜對,多了美麗與浪漫的情感' :
                        c.type === 'cannon'   ? '包炮相隔,多了渴望與情慾的能量' :
                        c.type === 'king'     ? '帥將王見王,勢均力敵' : '同字異色的好朋友';
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    const drained = Object.entries(map).filter(([_,n]) => n >= 2)
                        .map(([k]) => k.split('_')[0]);
                    return drained.length ? { pieces: drained } : null;
                },
                narrate(m, p) {
                    const c = p.center;
            ...
            },
            {
                ...
                detect(p) {
                    const map = {};
                    Object.values(p).forEach(x => {
                        map[x.name] = map[x.name] || { red: 0, black: 0 };
                        map[x.name][x.color]++;
                    });
                    for (const name in map) {
                        const { red, black } = map[name];
                        if ((red === 1 && black === 2) || (red === 2 && black === 1)) {
                            return { piece: name, redCount: red, blackCount: black };
                        }
                    }
                    return null;
                },
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    return allDevoured ? { count: others.length } : null;
                },
                narrate() {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    for (const [fp, atk] of Object.entries(p)) {
                        ...
                        if (canAttack(fp, 'center', atk, p) &&
                            !hasProtector('center', p) &&
                            !isPinned(fp, p)) {
                            return { byPos: fp, byPiece: atk.name };
                        }
                    }
                    return null;
                },
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (reds >= 3) return { kind: '兵', count: reds };
                    if (blacks >= 3) return { kind: '卒', count: blacks };
                    return null;
                },
                narrate(m) {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (p.left.color === c && p.right.color === c) return { axis: 'horizontal' };
                    if (p.up.color === c && p.down.color === c) return { axis: 'vertical' };
                    return null;
                },
                narrate(m) {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (c === p.left.color && c === p.right.color &&
                        c !== p.up.color && c !== p.down.color) {
                        return { variant: '235同色' };
                    }
                    return null;
                },
                narrate() {
                ...
                detect(p) {
                    ...
                    for (const fromPos of ['left','right','up','down']) {
                        ...
                        for (const toPos of Object.keys(p)) {
                            ...
                            matches.push({ from: fromPos, attacker: piece.name, target: target.name });
                        }
                    }
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (checkPair(p.left, p.right)) return { type: '正', axis: '左右' };
                    if (checkPair(p.up, p.down))   return { type: '正', axis: '上下' };
                    // 偏桃花:其他位置出現包/炮(且非中央)
                    const outers = ['left','right','up','down'].map(d => p[d]);
                    const hasC = outers.some(x => x.name === '炮');
                    const hasB = outers.some(x => x.name === '包');
                    if (hasC || hasB) return { type: '偏' };
                    return null;
                },
                narrate(m) {
                    if (m.type === '正') {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    const check = (a, b, axis) => {
                        ...
                        if (a.name === b.name) return { axis, name: a.name };
                        if (FRIEND_MAP[a.name] === b.name) return { axis, name: `${a.name}+${b.name}` };
                        return null;
                    };
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (outers.every(x => x !== c)) return { centerColor: c };
                    return null;
                },
                narrate(m, p, ctx) {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (diff.length === 1) return { dir: diff[0], piece: p[diff[0]] };
                    return null;
                },
                narrate(m) {
                    const dirName = { left:'左', right:'右', up:'上', down:'下' }[m.dir];
                    const piece = m.piece;
                    let extra = '';
                    ...
                    let posHint = '';
                    if (m.dir === 'up') posHint = '可探討是否有長輩或祖先的問題';
                    else if (m.dir === 'down') posHint = '可探討是否有晚輩或嬰靈的問題';
                    else posHint = '可探討是否有無緣未出生或往生平輩的問題';
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (colors.every(c => c === 'red')) return { color: 'red' };
                    if (colors.every(c => c === 'black')) return { color: 'black' };
                    return null;
                },
                narrate(m) {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    return found.length ? { positions: found } : null;
                },
                narrate() {
            ...
            },
            {
                ...
                detect(p) {
                    ...
                    if (total === 5 && ((reds === 4 && blacks === 1) || (reds === 1 && blacks === 4))) {
                        return { ratio: reds === 4 ? '4紅1黑' : '1紅4黑' };
                    }
                    if (total === 4 && ((reds === 3 && blacks === 1) || (reds === 1 && blacks === 3))) {
                        return { ratio: reds === 3 ? '3紅1黑' : '1紅3黑' };
                    }
                    return null;
                },
                narrate(m) {
                    const advice = m.ratio.startsWith('4紅')
                        ? '多踩泥土、草地(釋放陽能量),靜心冥想、抄經'
                        : '多曬太陽(出日落前後 15 分鐘是黃金時段),戶外活動接觸人群';
        ...
        const CHART_PERIODS = [
            { id: 'main',     label: '本命總格',     hint: '一生的主軸',         category: 'general' },
            { id: 'study',    label: '學習格 11~20歲',hint: '學業、修行',         category: 'general' },
            { id: 'love',     label: '情感格 21~30歲',hint: '感情、人際',         category: 'love'    },
            { id: 'career',   label: '事業格 31~40歲',hint: '工作、事業',         category: 'career'  },
            { id: '41-50',    label: '41~50 歲',     hint: '中壯年',             category: 'general' },
            { id: '51-60',    label: '51~60 歲',     hint: '事業熟成',           category: 'general' },
            { id: '61-70',    label: '61~70 歲',     hint: '收斂期',             category: 'general' },
            { id: '71-80',    label: '71~80 歲',     hint: '晚年',               category: 'general' }
        ];
        ...
        const EMPTY_PIECE = {
            name: '', color: '_empty', type: '_empty', score: 0,
            element: '', virtue: '', emotion: '', organ: '',
            rep: '', pos: '', neg: '',
            learnGood: '', learnBad: '',
            loveGood: '', loveBad: '',
            careerGood: '', careerBad: '',
            money: ''
        };
        ...
        const buildInsights = (ids, trend, c) => {
            ...
            // ── 助力類 ──
            if (has('wealth'))         out.push({ kind:'pos', text:`身邊正流動著貴人的能量 ─ 有人會在關鍵時刻主動相助,您不需要事事親力親為。` });
            if (has('career'))         out.push({ kind:'pos', text:`卦中帶著「認真做事」的氣場,您具備行動力與承擔的能量,這是您此刻最重要的底氣。` });
            if (has('good_friend'))    out.push({ kind:'pos', text:`身邊有「同道之人」的支持,這份連結讓您不會孤單前行,也不會被全然吃光。` });
            if (has('good_relations')) out.push({ kind:'pos', text:`您的人脈與緣分正在流動,願意走出去、與人接觸,機會就藏在人群裡。` });
            if (has('mutual_admire'))  out.push({ kind:'pos', text:`您與身邊某位夥伴有著相互欣賞的能量,這份共振是難得的禮物。` });
            if (has('three_united'))   out.push({ kind:'pos', text:`您已經準備好了 ─ 內在的底氣、外在的時機都到位,可以放心啟動醞釀已久的計畫。` });
            if (has('cross_help'))     out.push({ kind:'pos', text:`卦象帶有天助的能量,逢凶可化吉,有看不見的力量在支持著您。` });
            if (has('umbrella'))       out.push({ kind:'pos', text:`如同在父母或長輩的傘下,有照拂、有安定;但也提醒您,別讓這份庇護限制了您看天空的視野。` });
            if (has('victory'))        out.push({ kind:'pos', text:`您想做的事相對容易成局,行動是這次的關鍵。` });
            if (has('wise_ruler'))     out.push({ kind:'pos', text:`您此刻情緒安定、願意傾聽,這份「人和」的能量會帶來意想不到的助力。` });
            if (has('spiritual_affinity')) out.push({ kind:'pos', text:`您本身帶著靈動、能跨越障礙的能量,且這份能量正向外發揮 ─ 是修行緣分深、適合靜心學習的時節,功課與修行能事半功倍。` });

            // ── 中性/主題類 ──
            if (has('romance'))        out.push({ kind:'mid', text:`卦中帶有桃花的能量,情感會有強烈的吸引;只是要記得分清正緣與偏緣,別讓激情主導長期的方向。` });
            if (has('depressed_or_starsmoon') && trend === 'sowing') out.push({ kind:'mid', text:`外人不一定看得出您的鬱悶 ─ 主動找信任的人聊聊,別獨自撐著。` });
            if (has('depressed_or_starsmoon') && trend !== 'sowing') out.push({ kind:'mid', text:`外人很看好您的狀態,但內心仍可能有些壓力,記得給自己留下喘息的空間。` });
            if (has('devour_all'))     out.push({ kind:'mid', text:`您現在能掌控全局,但「全部拿下」的局面在最後容易歸零;留些餘地給對方,反而能走得更長遠。` });

            // ── 留意類 ──
            if (has('drain'))          out.push({ kind:'neg', text:`能量場中有些「消耗」的痕跡,先辨認消耗從何而來 ─ 是內在的自我懷疑,還是外人的拖累 ─ 看見它,才能轉化它。` });
            if (has('destroy'))        out.push({ kind:'neg', text:`原本應是助力的能量被消耗破壞了,您可能需要主動修復某段關係,或調整自己的內耗模式。` });
            if (has('devoured'))       out.push({ kind:'neg', text:`外在的力量比較強,您此刻需要先設好防線、降低暴露面,再做下一步決定 ─ 別硬碰硬。` });
            if (has('separation'))     out.push({ kind:'neg', text:`您與身邊某位的價值觀有些分歧,先溝通、先對齊期待,再談下一步。` });
            if (has('dilemma'))        out.push({ kind:'neg', text:`您正站在搖擺的人生路口 ─ 別在資訊不全時硬選,先沉澱、再做決定。` });
            if (has('lone_flower'))    out.push({ kind:'neg', text:`近期請留意人際邊界,可能會遇到讓您損失的人事物;也記得照顧好自己的能量,適度淨化或減少不必要的接觸。` });
            if (has('all_same_color')) out.push({ kind:'neg', text:`卦象呈現得有些單一,可能是您此刻的意識顯化動能不足。可以先做一些自我療癒,稍後再卜也是好的。` });
            if (has('tyrant'))         out.push({ kind:'neg', text:`提醒您警覺自己是否陷入了「孤君」的狀態 ─ 主動聆聽外界的聲音,別硬撐到底。` });
            if (has('yin_yang_imbalance')) out.push({ kind:'neg', text:`您此刻的陰陽能量稍微失衡,情緒波動會比較大;先做能量平衡(曬曬太陽,或踩踩泥土草地),再面對眼前的問題會更輕鬆。` });
            if (has('no_pawn'))        out.push({ kind:'neg', text:`卦中缺少踏實的能量,提醒您先做一兩件可以落地的小事,把腳步踩穩,再談大方向。` });
        ...
        const pickPositionHint = (pieces, posDef, category, c) => {
            ...
            if (category === 'love') {
                let dir = null;
                if (posDef.right.includes('夫') || posDef.right.includes('男友')) dir = 'right';
                else if (posDef.left.includes('妻') || posDef.left.includes('女友')) dir = 'left';
                if (dir) {
                    const p = pieces[dir];
        ...
        const detectCrossPeriodInfluence = (periodId, allCrosses) => {
            const out = { upward: [], lifeAndDeath: null };
            ...
            if (upPiece && upPiece.color !== '_empty' && upPiece.color !== c.color &&
                canAttack('center', 'up', c, cur) &&
                !hasProtector('up', cur) &&
                !isPinned('center', cur)) {
            ...
            if (upPiece && upPiece.color !== '_empty' && upPiece.color !== c.color &&
                canAttack('up', 'center', upPiece, cur) &&
                !hasProtector('center', cur) &&
                !isPinned('up', cur)) {
                out.lifeAndDeath = {
                    by: upPiece.name,
                    target: c.name,
        ...
        const composeChartFullNarrative = (pieces, opts) => {
            const { posDef, periodLabel, isMain, periodId, allCrosses } = opts;
            ...
            const trend =
                ratio >  scoreCtx.sacrifice * 0.3 ? 'harvest' :
                ratio < -scoreCtx.harvest   * 0.3 ? 'sowing'  : 'balanced';
            ...
            if (harvestModifierReasons.length) {
                harvestModifierReasons.forEach(r => { text += `※ ${r}\n`; });
            }
            ...
            if (cpInfluence.upward.length || cpInfluence.lifeAndDeath) {
                ...
                cpInfluence.upward.forEach(u => {
                    ...
                    const verb =
                        u.attackerType === 'rook'   ? '直線隔空往上' :
                        u.attackerType === 'cannon' ? '隔山往上' :
                        u.attackerType === 'knight' ? '完整吃過' :
                        u.attackerType === 'pawn'   ? '步步往上' : '繼續往上';
            ...
            if (isMain) {
                ...
                const lacks = [];
                if (!tiers.has('天')) lacks.push({ k:'天', text:'缺天 ─ 較鐵齒,相信「人定勝天」,眼見為憑,不易仰賴更大的力量' });
                if (!tiers.has('人')) lacks.push({ k:'人', text:'缺人 ─ 人和方面較弱,不易交到知心朋友,需在人際關係多用心' });
                if (!tiers.has('地')) lacks.push({ k:'地', text:'缺地 ─ 缺踏實感,人生易感迷惘,錢財較留不住,需主動落地' });
                if (lacks.length) {
                    text += `【天人地補充】\n`;
                    lacks.forEach(l => { text += `・ ${l.text}\n`; });
                    text += `\n`;
                }
                ...
                if (allPawn && !allFiveAllPawn) {
                    const masterDetail = {
                        king:    { line:'帥/將 → 做任何行業都適合,放手嘗試都能開花', why:'本質為天格之王,周圍踏實兵卒承載 ─ 想做什麼,落地都會有成果' },
                        guard:   { line:'仕/士 → 適合從政、掌權、處理複雜人事',     why:'仕士本質為策畫與輔佐,踏實兵卒帶來執行力 ─ 善用人脈與制度,影響力會放大' },
                        minister:{ line:'相/象 → 為一代宗師,適合教學、傳承、文化專業', why:'相象本質為智慧與沉澱,周圍兵卒落地 ─ 越專注於專業,越能成大師' },
                        rook:    { line:'俥/車 → 適合領導一群人做事,擅長執行型事業', why:'俥車本質為衝鋒,周圍兵卒呼應 ─ 帶兵打仗最自在,當掌舵者最發揮' },
                        knight:  { line:'傌/馬 → 會有一群人跟他到處奔波,適合業務、外交、開疆拓土', why:'傌馬本質為跳躍與遠行,兵卒護持 ─ 越走動越旺,別固守一地' },
                        cannon:  { line:'炮/包 → 因男的帥女的美,適合當偶像明星、表演、形象工作', why:'炮包本質為爆發與吸引,踏實兵卒承載光環 ─ 鏡頭前後都自帶氣場' },
                        pawn:    { line:'兵/卒 → 五位皆地,即「皇帝格」,只往上不會往下',           why:'已是皇帝格,見下方說明' }
                    };
            ...
            const isCareerEstSeg = isMain ||
                periodId === 'career' || periodId === '41-50' || periodId === '51-60';
            if (isCareerEstSeg && adjHarvest > 0) {
                const wan = Math.round(adjHarvest * 25);
                const estStr = wan >= 10000
                    ? `約 ${(wan / 10000).toFixed(1)} 億`
                    : wan >= 1000
                        ? `約 ${(wan / 1000).toFixed(1)} 千萬`
                        : `約 ${wan} 萬`;
                const scope = isMain ? '一生'
                            : periodId === 'career' ? '31~40 歲事業期'
                            : periodId === '41-50'  ? '41~50 歲'
                            : '51~60 歲';
                careerLines.push(
                    `${scope}賺錢粗估 ─ ${estStr} 元(收穫分數 ${adjHarvest.toFixed(1)} × 約 25 萬/分)。\n  此為《命盤解說》教材的概略尺度,實際金額因人因業而異;倍數差距因「行業選對與否」、「努力強度」、「合作關係」而調整,別把數字看死。`
                );
            }
            ...
            if (!isMain && (periodLabel?.includes('情感') || periodLabel?.includes('事業'))) {
                ...
                if (marriageSignals.length) {
                    const ageRange = periodLabel.includes('21~30') ? '21~30 歲' :
                                     periodLabel.includes('31~40') ? '31~40 歲' : '此段';
                    text += `【💍 婚姻時機提示】\n`;
            ...
            if (cannonPositions.length) {
                const isProperRomance =
                    (pieces.left.type === 'cannon' && pieces.right.type === 'cannon') ||
                    (pieces.up.type === 'cannon' && pieces.down.type === 'cannon');
                loveLines.push(isProperRomance
                    ? `卦中包炮在對位,屬正桃花能量 ─ 容易遇到合適的對象。`
                    : `卦中有包/炮但不在正位,屬偏桃花 ─ 情慾能量強,需理性對待。`);
            }
            ...
            if (has('yin_yang_imbalance')) {
                ...
                healthLines.push(reds >= 4
                    ? `※ 陰陽不協調(陽過盛):多踩泥土草地、靜心冥想、抄經以收陽氣。`
                    : `※ 陰陽不協調(陰過盛):多曬太陽、出門接觸人群以補陽。`);
            }
            // 5. 五行所缺
            ...
            // 6. 五行過多
            const elemCount = {};
            elements.forEach(e => { elemCount[e] = (elemCount[e]||0)+1; });
        ...
        const composeNarrative = (pieces, opts) => {
            const { category, posDef, question, periodLabel, briefMode } = opts;
            const c = pieces.center;
            const catName = { career:'事業工作', wealth:'金錢財運', love:'感情婚姻', general:'整體運勢' }[category] || '整體運勢';
        ...
        const MiniPieceSlot = ({ piece, onClick, isCenter, position }) => {
            const POS_LABEL = { center:'中', left:'左', right:'右', up:'上', down:'下' };
        ...
        const ChessPiece = ({ piece, label, x, y, isCenter, isShuffling }) => {
            ...
            if (isShuffling) {
                return (
                    <div className={`chess-piece shuffling-piece ${colorClass} bg-[#fffdf5]`}>
                        <span className="drop-shadow-sm mt-[-2px]">{piece.name}</span>
                    </div>
                );
            }
            ...
            return (
                <div className="absolute flex flex-col items-center group" style={style}>
                    <div className={`chess-piece ${colorClass} ${bgClass}`} style={{ position: 'relative' }}>
                        <span className="drop-shadow-sm mt-[-2px]">{piece.name}</span>
                    </div>
                    {label && (
                        <span className={`absolute top-[80px] sm:top-[94px] text-[11px] font-bold tracking-widest px-3 py-0.5 rounded-full whitespace-nowrap shadow-sm ${isCenter ? 'text-red-800 bg-red-50 border border-red-200' : 'text-stone-500 bg-white/85 border border-stone-200'}`}>
                            {label}
                        </span>
                    )}
                </div>
        ...
        const FortuneTeller = () => {
            ...
            const getDeck = () => {
                const deck = [];
                const counts = { r_king:1, b_king:1, r_guard:2, b_guard:2, r_minister:2, b_minister:2, r_rook:2, b_rook:2, r_knight:2, b_knight:2, r_cannon:2, b_cannon:2, r_pawn:5, b_pawn:5 };
                Object.keys(counts).forEach(key => {
                    for(let i=0; i<counts[key]; i++) deck.push({ ...CHESS_DB[key], uid: `${key}_${i}` });
                });
                return deck;
            };
            ...
            const mentionsOtherPerson = (text) => {
                const cleaned = text
                    .replace(/其他/g, '').replace(/吉他/g, '')
                    .replace(/他山之石/g, '').replace(/他鄉/g, '').replace(/她鄉/g, '');
            ...
            const handleStart = async () => {
                if (!gender)   { alert("⚠️ 請先選擇您的性別"); return; }
                if (!category) { alert("⚠️ 請先選擇問題類別"); return; }
                if (!question.trim()) { alert("⚠️ 請輸入您的問題"); return; }
                if (question.length < 5) { alert("⚠️ 問題太短,請包含具體事項與時間"); return; }
            ...
            const handlePickCard = async (cardIdx) => {
                ...
                const fullText = composeNarrative(res, { category, posDef, question });
            ...
            const handleGenerateChart = async () => {
                if (!gender)  { alert("⚠️ 請先選擇您的性別"); return; }
                if (!marital) { alert("⚠️ 請選擇婚姻狀態"); return; }
                const y = parseInt(birthYear, 10);
                const m = parseInt(birthMonth, 10);
                const d = parseInt(birthDay, 10);
                if (!y || y < 1900 || y > 2100) { alert("⚠️ 請輸入正確的西元出生年份"); return; }
                if (!m || m < 1 || m > 12)      { alert("⚠️ 請輸入正確的月份(1-12)"); return; }
                if (!d || d < 1 || d > 31)      { alert("⚠️ 請輸入正確的日期(1-31)"); return; }
            ...
            const updateManualPiece = (periodId, position, key) => {
                ...
                setManualPieces(prev => ({
                    ...prev,
                    [periodId]: { ...(prev[periodId] || {}), [position]: key }
                }));
            };
            const handleSubmitManualChart = async () => {
                ...
                PERIOD_ORDER.forEach((periodId, idx) => {
                    ...
                    for (const pos of PERIOD_UNIQUE_SLOTS[periodId]) {
                        ...
                        cross[pos] = { ...CHESS_DB[key], uid: `${periodId}_${pos}_${key}` };
                    }
                    if (periodId === '71-80') {
            ...
            const getPosition = (pos) => {
                ...
                switch(pos) {
                    ...
                    case 'left':   return { x: -spacing, y: 0 };
                    case 'right':  return { x: spacing, y: 0 };
                    case 'up':     return { x: 0, y: -spacing };
                    case 'down':   return { x: 0, y: spacing };
                    default: return { x: 0, y: 0 };
                }
            };

            return (
                <div className="min-h-screen flex flex-col items-center justify-center p-4 overflow-y-auto relative">
                    <div className="text-center mb-4 z-10">
                        <h1 className="text-4xl font-black text-stone-800 border-b-4 border-red-900 pb-2 inline-block tracking-widest">觀棋心徑</h1>
                        <p className="text-sm text-stone-500 mt-2 font-bold">JackyChess 線上象棋卜卦系統</p>
                    </div>

                    {/* 模式切換 (只在 input 階段顯示) */}
                    {stage === 'input' && (
                        <div className="flex gap-2 mb-5 z-20 bg-white/80 p-1 rounded-full shadow-md border border-stone-200">
                            <button
                                onClick={() => { if (mode !== 'single') { handleReset(true); setMode('single'); } }}
                            ...
                            <button
                                onClick={() => { if (mode !== 'chart') { handleReset(true); setMode('chart'); } }}
                    ...
                    {/* === 單卦 輸入表單 === */}
                    {stage === 'input' && mode === 'single' && (
                        <div className="w-full max-w-md animate-pop bg-white/90 p-6 rounded-xl shadow-xl z-50">
                            <p className="mb-2 text-stone-800 font-black text-base">1. 您的性別:</p>
                            <div className="grid grid-cols-2 gap-3 mb-4">
                                <button onClick={() => setGender('male')} className={`cat-btn py-2 rounded-lg font-bold ${gender==='male'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>♂ 男生</button>
                                <button onClick={() => setGender('female')} className={`cat-btn py-2 rounded-lg font-bold ${gender==='female'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>♀ 女生</button>
                            </div>

                            <p className="mb-2 text-stone-800 font-black text-base">2. 問題類別:</p>
                            <div className="grid grid-cols-2 gap-3 mb-5">
                                <button onClick={() => handleCategoryChange('career')}  className={`cat-btn py-3 rounded-lg font-bold ${category==='career'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>💼 事業工作</button>
                                <button onClick={() => handleCategoryChange('wealth')}  className={`cat-btn py-3 rounded-lg font-bold ${category==='wealth'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>💰 金錢財運</button>
                                <button onClick={() => handleCategoryChange('love')}    className={`cat-btn py-3 rounded-lg font-bold ${category==='love'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>❤️ 感情婚姻</button>
                                <button onClick={() => handleCategoryChange('general')} className={`cat-btn py-3 rounded-lg font-bold ${category==='general'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>🔮 整體運勢</button>
                            </div>

                            <p className="mb-2 text-stone-800 font-black text-base">3. 心中疑惑 <span className="text-xs text-stone-500 font-normal">(已自動帶入範例,可自行修改)</span>:</p>
                            <div className="bg-red-50 border-l-4 border-red-500 p-3 mb-3 text-xs text-red-700 leading-relaxed font-bold">
                                ⚠️ 提問規則:<br/>
                                1. 只能問「您自己」的事(直系血親除外)。<br/>
                                2. 請包含具體的「事件」與「時間」。<br/>
                                3. 卦象只給方向不給絕對答案。
                            </div>
                            <textarea className="w-full p-3 border-2 border-stone-300 rounded-lg mb-1 h-28 focus:border-red-800 outline-none text-stone-700 bg-white"
                                placeholder="請選擇上方類別,系統會自動帶入該類別的範例問題;您也可以自行替換成自己的問題。"
                                maxLength={150}
                                value={question} onChange={e => setQuestion(e.target.value)} />
                            <div className={`text-right text-xs mb-3 font-bold ${question.length >= 150 ? 'text-red-700' : (question.length >= 130 ? 'text-amber-600' : 'text-stone-400')}`}>
                                {question.length} / 150 字
                            </div>
                            <button onClick={handleStart} className="w-full py-4 bg-red-900 text-white rounded-xl font-bold text-xl shadow-lg hover:bg-red-800 active:scale-95 transition-all">開始洗牌</button>
                        </div>
                    )}

                    {/* === 命盤 輸入表單 === */}
                    {stage === 'input' && mode === 'chart' && (
                        <div className="w-full max-w-md animate-pop bg-white/90 p-6 rounded-xl shadow-xl z-50">
                            <p className="text-xs text-stone-500 mb-4 leading-relaxed">
                                命盤呈現您一生的格局,讓我們在每個轉折時刻,都能有方向、有準備。<br/>
                                一旦推算便如塵埃落定,<span className="text-red-700 font-bold">不可再重抽</span> ─ 請懷著慎重的心,如實輸入您的出生資料。
                            </p>

                            {/* 輸入方式切換 (2 選 1) */}
                            <div className="grid grid-cols-2 gap-1 mb-5 p-1 bg-stone-100 rounded-full">
                                <button
                            ...
                            {chartInputMethod === 'draw' && (
                                <p className="text-xs text-stone-500 bg-stone-50 border-l-4 border-red-300 p-3 mb-4 leading-relaxed">
                                    凝神之後,依序抽 32 張棋,系統會自動依「總格→十年運」順序排入命盤。
                                </p>
                            )}
                            {chartInputMethod === 'manual' && (
                                <p className="text-xs text-stone-500 bg-stone-50 border-l-4 border-stone-300 p-3 mb-4 leading-relaxed">
                                    若您已從老師處取得命盤,可在此自行輸入(共 32 個唯一棋位),看本系統如何解盤。
                                </p>
                            )}
                            ...
                            <p className="mb-2 text-stone-800 font-black text-base">2. 婚姻狀態:</p>
                            <div className="grid grid-cols-2 gap-3 mb-4">
                                <button onClick={() => setMarital('single')} className={`cat-btn py-2 rounded-lg font-bold ${marital==='single'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>未婚</button>
                                <button onClick={() => setMarital('married')} className={`cat-btn py-2 rounded-lg font-bold ${marital==='married'?'active text-red-900':'bg-stone-50 text-stone-600'}`}>已婚</button>
                            </div>

                            {chartInputMethod === 'draw' && (
                                <button onClick={handleStartChartDraw} className="w-full py-4 bg-red-900 text-white rounded-xl font-bold text-xl shadow-lg hover:bg-red-800 active:scale-95 transition-all">開始抽棋</button>
                            )}

                            {chartInputMethod === 'manual' && (
                                <>
                                    <p className="mb-2 text-stone-800 font-black text-base">3. 依教材排版輸入您的命盤:</p>
                                    <p className="text-xs text-stone-500 mb-3 leading-relaxed">
                                        ✓ 點選任一空白圓圈即可選擇棋子<br/>
                                        ✓ 已填的圓圈再次點選可更換<br/>
                                        ✓ 棋子顏色內建(帥仕相俥傌炮兵=紅,將士象車馬包卒=黑)
                                    </p>
                                    <div className="input-period-list mb-5">
                                        {CHART_PERIODS.map((period, pidx) => {
                                            ...
                                            const upPiece = period.id === 'main'
                                                ? (ownUpKey ? CHESS_DB[ownUpKey] : null)
                                                : (inheritedUpKey ? CHESS_DB[inheritedUpKey] : null);
                                            // 71-80 段的 down 位 = 總格 main.up(命盤循環),不可手動點選,直接顯示繼承棋子
                                            const downPiece = period.id === '71-80'
                                                ? (manualPieces.main?.up ? CHESS_DB[manualPieces.main.up] : null)
                                                : (manualPieces[period.id]?.down ? CHESS_DB[manualPieces[period.id].down] : null);
                                            return (
                                                <div key={period.id} className="input-period-row">
                                                    <div className="info">
                                                        <div className="row-title">{pidx === 0 ? '★ ' : `${pidx}. `}{period.label}</div>
                                                        <div className="row-hint">{period.hint}</div>
                                                        <div className={`row-progress ${filled === need ? 'done' : 'todo'}`}>
                                                            {filled === need ? '✓ 已填完' : `已填 ${filled} / ${need}`}
                                                        </div>
                                                        {pidx > 0 && (
                                                    ...
                                                    <div className="cross-mini">
                                                        <div></div>
                                                        {period.id === 'main'
                                                            ? <MiniPieceSlot piece={upPiece} position="up" onClick={() => setPickerSlot({ periodId: period.id, position: 'up' })} />
                                                            : <MiniPieceSlot piece={upPiece} position="up" />}
                                                        <div></div>
                                                        <MiniPieceSlot piece={manualPieces[period.id]?.left && CHESS_DB[manualPieces[period.id].left]} position="left" onClick={() => setPickerSlot({ periodId: period.id, position: 'left' })} />
                                                        <MiniPieceSlot piece={manualPieces[period.id]?.center && CHESS_DB[manualPieces[period.id].center]} position="center" isCenter={true} onClick={() => setPickerSlot({ periodId: period.id, position: 'center' })} />
                                                        <MiniPieceSlot piece={manualPieces[period.id]?.right && CHESS_DB[manualPieces[period.id].right]} position="right" onClick={() => setPickerSlot({ periodId: period.id, position: 'right' })} />
                                                        <div></div>
                                                        {period.id === '71-80'
                                                            ? <MiniPieceSlot piece={downPiece} position="down" />
                                                            : <MiniPieceSlot piece={downPiece} position="down" onClick={() => setPickerSlot({ periodId: period.id, position: 'down' })} />}
                                                        <div></div>
                                                    </div>
                                                </div>
                                            );
                                    ...
                                    <button onClick={handleSubmitManualChart} className="w-full py-4 bg-red-900 text-white rounded-xl font-bold text-xl shadow-lg hover:bg-red-800 active:scale-95 transition-all">開始解盤</button>
                                </>
                            )}
                        </div>
                    )}

                    {/* 凝神過場(單卦 2 秒) / 排盤過場(命盤 2 秒) */}
                    {stage === 'ritual' && (
                        <div className="relative w-80 h-80 sm:w-96 sm:h-96 flex items-center justify-center mb-8 z-0">
                            <div className="mystic-aura"></div>
                            <div className="mystic-aura inner"></div>
                            <div className="center-glow"></div>
                            <div className="ritual-text px-4">
                                {mode === 'single' ? (
                                    <>
                                        <p className="text-stone-700 font-black text-lg sm:text-xl mb-3 tracking-widest">凝神中</p>
                                        <p className="text-stone-500 text-xs sm:text-sm leading-relaxed">
                                            請在心中默念<span className="dot-1">.</span><span className="dot-2">.</span><span className="dot-3">.</span><br/>
                                            <span className="text-stone-600 font-bold">「我想知道 {question.length > 0 ? question.trim().slice(0, 12) + (question.length > 12 ? '…' : '') : '___'}」</span><br/>
                                            <span className="text-stone-400">恭請指導靈給予指示</span>
                                        </p>
                                    </>
                                ) : (
                                    <>
                                        <p className="text-stone-700 font-black text-lg sm:text-xl mb-3 tracking-widest">排盤中</p>
                                        <p className="text-stone-500 text-xs sm:text-sm leading-relaxed">
                                            正在依您的出生時辰<br/>
                                            推算這一生的能量地圖<span className="dot-1">.</span><span className="dot-2">.</span><span className="dot-3">.</span>
                                        </p>
                                    </>
                                )}
                            </div>
                    ...
                    {stage === 'chart_picking' && (() => {
                        ...
                        return (
                            <div className="w-full max-w-md flex flex-col items-center mb-8 z-10 px-2">
                                <div className="text-center mb-4">
                                    <p className="text-stone-700 font-bold text-base mb-1">
                                        請依序抽 <span className="text-red-800">32 張</span> 棋
                                    </p>
                                    {nextSlot ? (
                                        <p className="text-red-700 text-sm font-bold">
                                            第 <span className="text-2xl mx-1">{pickedCount + 1}</span> 張 → 【{nextPeriodDef?.label}】「{POS_LABEL[nextSlot[1]]}」位
                                        </p>
                                    ) : (
                                        <p className="text-stone-500 text-sm font-bold">
                                            32 張已抽完,即將揭曉<span className="dot-1">.</span><span className="dot-2">.</span><span className="dot-3">.</span>
                                        </p>
                                    )}
                                    <p className="text-xs text-stone-400 mt-1">⚠️ 一旦選定,不可反悔</p>
                                </div>
                                <div className="grid grid-cols-8 gap-1.5 sm:gap-2 mb-3">
                                    {Array.from({ length: 32 }, (_, i) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                {selected && (
                                                    <span className="text-xs sm:text-base font-black">{order + 1}</span>
                                                )}
                                            </button>
                                        );
                                    })}
                                </div>
                                <p className="text-stone-400 text-xs">已抽 {pickedCount} / 32</p>
                            </div>
                        );
                    })()}
                    ...
                    {stage === 'picking' && (
                        <div className="w-full max-w-md flex flex-col items-center mb-8 z-10 px-2">
                            <div className="text-center mb-4">
                                <p className="text-stone-700 font-bold text-base mb-1">
                                    請依您的心念,依序選擇 <span className="text-red-800">5 張</span> 棋牌
                                </p>
                                {selections.length < 5 ? (
                                    <p className="text-red-700 text-sm font-bold">
                                        正在選第 <span className="text-2xl mx-1">{selections.length + 1}</span> 張(共 5 張)
                                    </p>
                                ) : (
                                    <p className="text-stone-500 text-sm font-bold">已選滿,即將揭曉<span className="dot-1">.</span><span className="dot-2">.</span><span className="dot-3">.</span></p>
                                )}
                                <p className="text-xs text-stone-400 mt-1">⚠️ 一旦選定,不可反悔</p>
                            </div>
                            <div className="grid grid-cols-8 gap-1.5 sm:gap-2 mb-3">
                                {Array.from({ length: 32 }, (_, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {selected && (
                                                <span className="text-2xl sm:text-3xl">{orderChar}</span>
                                            )}
                                        </button>
                                    );
                                })}
                            </div>
                            <p className="text-stone-400 text-xs">已選 {selections.length} / 5</p>
                        </div>
                    )}

                    {/* === 單卦結果(現有流程) === */}
                    {stage === 'result' && mode === 'single' && (
                        <div className="relative w-80 h-80 sm:w-96 sm:h-96 flex items-center justify-center mb-8 z-0">
                            <div className="absolute inset-0 border-2 border-dashed border-stone-300 rounded-full opacity-50"></div>
                            <div className="absolute inset-8 border border-dashed border-stone-200 rounded-full opacity-60"></div>
                            ...
                            {drawnPieces && (
                                <>
                                    {visiblePieces.includes('center') && <ChessPiece piece={drawnPieces.center} {...getPosition('center')} isCenter={true} />}
                                    {visiblePieces.includes('left')   && <ChessPiece piece={drawnPieces.left}   {...getPosition('left')}   />}
                                    {visiblePieces.includes('right')  && <ChessPiece piece={drawnPieces.right}  {...getPosition('right')}  />}
                                    {visiblePieces.includes('up')     && <ChessPiece piece={drawnPieces.up}     {...getPosition('up')}     />}
                                    {visiblePieces.includes('down')   && <ChessPiece piece={drawnPieces.down}   {...getPosition('down')}   />}
                                </>
                            )}
                        </div>
                    )}

                    {stage === 'result' && mode === 'single' && aiText && (
                        <div className="max-w-2xl w-full bg-white/95 p-6 rounded-xl shadow-2xl border-t-4 border-red-900 animate-pop mb-12 z-20">
                            <h2 className="text-xl font-black text-stone-800 mb-4 flex items-center"><span className="w-2 h-6 bg-red-900 mr-2 rounded"></span>卦象解析</h2>
                            <div className="whitespace-pre-line text-stone-800 leading-relaxed font-medium min-h-[100px] text-justify text-sm sm:text-base">
                                {aiText}
                            </div>
                            {showDonate && (
                                <div className="mt-8 animate-pop">
                                    <button onClick={() => window.open('https://portaly.cc/Jackychess', '_blank')}
                    ...
                    {/* === 命盤結果(垂直鏈 + 原地展開解讀)=== */}
                    {stage === 'result' && mode === 'chart' && chartCrosses && (() => {
                        ...
                        PERIOD_ORDER.forEach((id, idx) => {
                            ...
                            if (idx === 0) {
                                chainRows.push({ kind:'single', piece: cross.up, primary: id, label: def.label, isLabel: true });
                            }
                            chainRows.push({ kind:'triple', pieces:[cross.left, cross.center, cross.right], primary: id, label: def.label, isLabel: idx > 0 });
                            // 渲染下位:71-80 的下=主上,但仍渲染最後一行(代表循環)
                            chainRows.push({ kind:'single', piece: cross.down, primary: id, secondary: PERIOD_ORDER[idx+1] || null });
                        });
                        ...
                        return (
                            <div className="w-full max-w-3xl flex flex-col items-center z-20">
                                {/* 基本資訊 */}
                                <div className="text-center mb-3 text-xs text-stone-500">
                                    {gender === 'male' ? '♂ 男' : '♀ 女'} ・ {marital === 'married' ? '已婚' : '未婚'}
                                </div>
                                <p className="text-xs text-stone-500 text-center mb-3">
                                    點選任一段查看詳細解析 ─ 上一段的下位即下一段的上位(共用同一棋)
                                </p>

                                {/* === 主版面:左側垂直鏈 + 右側解讀(行動 stack)=== */}
                                <div className="w-full flex flex-col lg:flex-row gap-4 mb-6">
                                    {/* 命盤垂直鏈 */}
                                    <div className="lg:w-2/5 flex-shrink-0 bg-white/85 rounded-xl shadow-lg p-3 relative">
                                        <p className="text-xs font-bold text-stone-500 text-center mb-2 tracking-wider">─ 命盤(教材排法)─</p>
                                        <div className="chart-chain-wrap">
                                            <div className="chart-chain">
                                                {chainRows.map((row, ri) => {
                                                    ...
                                                    if (row.kind === 'single') {
                                                        ...
                                                        const colorClass = piece?.color === 'red' ? 'red' : 'black';
                                                        return (
                                                            <div key={ri} className={`chain-row ${hl}`} onClick={onClick}>
                                                                <div className={`chain-piece ${colorClass}`}>{piece?.name}</div>
                                                            </div>
                                                        );
                                                    }
                                                    return (
                                                        <div key={ri} className={`chain-row ${hl}`} onClick={onClick}>
                                                            {row.pieces.map((p, pi) => {
                                                                ...
                                                                return <div key={pi} className={`chain-piece ${cc}`}>{p?.name}</div>;
                                                            })}
                                                        </div>
                                                    );
                                                })}
                                            </div>
                                            <p className="chain-cycle-hint">↻ 最後一棋接回最上方第一棋(命盤循環)</p>
                                        </div>
                                        {/* 段落快選按鈕(底部) */}
                                        <div className="mt-3 pt-3 border-t border-stone-200">
                                            <p className="text-[10px] text-stone-400 text-center mb-2">─ 段落快選 ─</p>
                                    ...
                                    {/* 該段詳細解讀 */}
                                    <div className="lg:flex-1 bg-white/95 p-5 rounded-xl shadow-2xl border-t-4 border-red-900 animate-pop">
                                        <div className="mb-3">
                                            <h2 className="text-xl font-black text-stone-800 inline-block border-b-2 border-red-900 pb-1">
                                                {activeDef.label}
                                            </h2>
                                            <p className="text-xs text-stone-500 mt-1">{activeDef.hint}</p>
                                        </div>
                                        <div className="whitespace-pre-line text-stone-800 leading-relaxed font-medium text-justify text-sm sm:text-base">
                                            {activeNarrative}
                                        </div>
                                    </div>
                                </div>
                    ...
                    {pickerSlot && (() => {
                        ...
                        const POS_LABEL = { center:'中', left:'左', right:'右', up:'上' };
                        // 計算除了當前 slot 外其他位置已用了多少同名棋子
                        const usage = computeUsage(manualPieces, pickerSlot.periodId, pickerSlot.position);
                        const renderRow = (keys, colorClass) => (
                            <div className="picker-grid mb-2">
                                {keys.map(key => {
                                    ...
                                    return (
                                        <div key={key}
                                            ...
                                            <span className="picker-piece-name">{p.name}</span>
                                            <span className="picker-piece-count">{remaining}/{limit}</span>
                                        </div>
                                    );
                                })}
                            </div>
                        ...
                        return (
                            <div className="modal-overlay" onClick={() => setPickerSlot(null)}>
                                <div role="dialog" aria-modal="true" aria-label="選擇棋子"
                                     className="bg-white p-5 rounded-xl shadow-2xl max-w-md w-11/12 relative animate-pop"
                                     onClick={e => e.stopPropagation()}>
                                    <button aria-label="關閉" className="absolute top-2 right-3 text-3xl text-stone-400 hover:text-stone-600" onClick={() => setPickerSlot(null)}>&times;</button>
                                    <h3 className="text-base font-black text-stone-800 mb-1 text-center">選擇棋子</h3>
                                    <p className="text-xs text-stone-500 text-center mb-3">
                                        【{periodDef?.label}】 / 「{POS_LABEL[pickerSlot.position]}」位
                                    </p>
                                    <p className="text-[10px] text-stone-400 text-center mb-3">
                                        每顆棋子下方顯示「剩餘 / 牌庫總數」 ─ 用完即無法再選
                                    </p>
                                    {renderRow(['r_king','r_guard','r_minister','r_rook','r_knight','r_cannon','r_pawn'], 'red')}
                                    {renderRow(['b_king','b_guard','b_minister','b_rook','b_knight','b_cannon','b_pawn'], 'black')}
                                </div>
                            </div>
                        );
                    })()}
                    ...
                    {showContactModal && (
                        <div className="modal-overlay" onClick={() => setShowContactModal(false)}>
                            <div className="bg-white p-6 rounded-xl shadow-2xl max-w-sm w-10/12 text-center relative animate-pop" onClick={e => e.stopPropagation()}>
                                <button className="absolute top-2 right-3 text-3xl text-stone-400 hover:text-stone-600" onClick={() => setShowContactModal(false)}>&times;</button>
                                <h3 className="text-xl font-black text-stone-800 mb-4 border-b pb-2">預約深度諮詢</h3>
                                <div className="bg-stone-50 p-2 border border-stone-200 rounded-lg inline-block mb-3">
                                    <img src="https://qr-official.line.me/gs/M_587wduxm_GW.png?oat_content=qr" alt="Line QR" className="w-40 h-40 object-contain" />
                                </div>
                                <p className="text-xs text-stone-500 mb-4">掃描或點擊下方按鈕加入官方 LINE</p>
                                <div className="flex flex-col gap-3">
                                    <button onClick={() => window.open('https://line.me/R/ti/p/%40587wduxm', '_blank')} className="w-full py-3 bg-[#06c755] text-white rounded-lg font-bold shadow hover:bg-[#05b34c] transition flex items-center justify-center gap-2">
                                        <span>LINE</span> 加入好友預約
                                    </button>
                                    <button onClick={handleIG} className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white rounded-lg font-bold shadow hover:opacity-90 transition flex items-center justify-center gap-2">
        ...
        class ErrorBoundary extends React.Component {
            ...
            render() {
                if (this.state.hasError) {
                    return (
                        <div style={{padding:'40px 20px',textAlign:'center',fontFamily:"'Noto Serif TC', serif",color:'#7f1d1d',lineHeight:1.7,maxWidth:'480px',margin:'40px auto'}}>
                            <h2 style={{fontSize:'20px',marginBottom:'12px',fontWeight:900}}>頁面遇到錯誤</h2>
                            <p style={{color:'#78716c',fontSize:'13px',marginBottom:'8px'}}>請重新整理頁面再試一次。</p>
                            <p style={{color:'#a8a29e',fontSize:'11px',marginBottom:'20px',wordBreak:'break-word'}}>{this.state.msg}</p>
                            <button onClick={() => window.location.reload()}
                                style={{padding:'10px 28px',background:'#7f1d1d',color:'white',border:'none',borderRadius:'10px',fontWeight:'bold',cursor:'pointer',fontSize:'14px'}}>
                                ↻ 重新整理
                            </button>
                        </div>
                    );
        ...
        root.render(<ErrorBoundary><FortuneTeller /></ErrorBoundary>);